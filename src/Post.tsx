            ...
            <div>
                <div>
                    ...
                    <div className="post-detail-comment-body">
                        ...
                        <p className="post-detail-comment-content" data-e2e="comment-content">
                        {comment.content}
                        </p>
                        {commenting && (
                            <CreateComment
...
function CreateComment({
    ...
    return (
        <>
            <form className="post-detail-create-comment-form" data-e2e="create-comment-form" 
            onSubmit={(event) =>{
                ...
                .then(({error}) => {
                    if(error){
                        ...
                        onSuccess();
                        textareaRef.current?.value != null && (textareaRef.current.value = "");
                    }
                })
            }}>
                ...
                <div className="flex gap-2">
                    <button type="submit" className="post-detail-create-comment-form-submit-button" disabled={!comment}>
                        Envoyer
                    </button>
                    {onCancel && (
                        <button type="button" className="post-detail-create-comment-form-cancel-button" onClick={()=> onCancel()}>
                            Précédent
                        </button>
                    )}
                </div>
...
export function unsortedCommentsToNested(
    ...
    const sortedByDepthThenCreationTime = [...Object.values(commentMap)].sort(
        (a,b) => {
            ...
            return aDepth > bDepth ? 1 : aDepth < bDepth ? -1 : new Date(a.created_at).getTime() - new Date(b.created_at).getTime();
        }
    );
